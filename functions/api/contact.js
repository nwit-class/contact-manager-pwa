...
/**
 * POST /api/contacts
 * body: { since: <number>, push: [contacts...] }
 * returns: { now, pull: [contacts...] }
 */
export async function onRequestPost({ request, env }) {
  try {
    ...
    // Upsert pushed contacts
    for (const c of push) {
      const {
        uuid, name, email, phone, address, tags, notes,
        favorite = false, createdAt = now, updatedAt = now, deletedAt = null
      } = c;

      // Normalize
      const _tags = JSON.stringify(Array.isArray(tags) ? tags : []);

      await env.DB.prepare(`
        INSERT INTO contacts (
          user_id, uuid, name, email, phone, address, tags, notes,
      ...
      `).bind(
        userId, uuid, name || '', email || '', phone || '', address || '',
        _tags, notes || '', favorite ? 1 : 0, createdAt, updatedAt, deletedAt
      ).run();
    }

    // Pull anything changed after "since"
    const rows = await env.DB.prepare(`
      SELECT uuid, name, email, phone, address, tags, notes,
             favorite, created_at, updated_at, deleted_at
      FROM contacts
